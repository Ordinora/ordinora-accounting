...
async function post(kind:"SALE"|"PURCHASE",formData:FormData){
  ...
  await postCreditNote({kind,actor:{tenantId:active.id,userId:user.id,firmId:user.firmId,role:user.staffRole},documentId:h.documentId,reference,creditDate:h.creditDate,description:h.description,lines});redirect(withTransactionNotice(kind==="SALE"?"/sales/credit-notes":"/purchases/credit-notes","credit-note"));
}
