...
export async function createJournal(formData: FormData) {
  ...
  redirect(withTransactionNotice(`/journals/${journal.id}`, "manual-journal"));
}

async function move(formData: FormData, target: "IN_REVIEW" | "APPROVED" | "POSTED") { const { user, active } = await requireActiveTenantForMutation(); const id = z.string().min(1).parse(formData.get("journalId")); await transitionJournal({ tenantId: active.id, userId: user.id, role: user.staffRole }, id, target); revalidatePath(`/journals/${id}`); revalidatePath("/journals"); if(target==="POSTED")redirect(withTransactionNotice(`/journals/${id}`,"manual-journal")); }
...
export async function reversePostedJournal(formData: FormData) { const { user, active } = await requireActiveTenantForMutation(); const id = z.string().min(1).parse(formData.get("journalId")); const reason = z.string().min(5).max(240).parse(formData.get("reason")); await reverseJournal({ tenantId: active.id, userId: user.id, role: user.staffRole }, id, reason); redirect("/journals"); }
export async function deleteJournalPermanently(formData: FormData) { const { user, active } = await requireActiveTenantForMutation(); const id = z.string().min(1).parse(formData.get("journalId")); const confirmation = z.string().trim().min(1).max(80).parse(formData.get("confirmation")); const reason = z.string().trim().min(5).max(240).parse(formData.get("reason")); await permanentlyDeleteJournal({ tenantId: active.id, userId: user.id, role: user.staffRole }, id, confirmation, reason); revalidatePath("/"); revalidatePath("/journals"); revalidatePath("/purchases"); revalidatePath("/payments"); revalidatePath("/sales"); revalidatePath("/receipts"); revalidatePath("/reports"); redirect("/journals"); }
export async function editManualJournal(formData: FormData) { const { user, active } = await requireActiveTenantForMutation(); const data = z.object({ journalId: z.string().min(1), reference: z.string().trim().min(1).max(40), description: z.string().trim().min(3).max(240), accountingDate: z.coerce.date(), debitAccountId: z.string().min(1), creditAccountId: z.string().min(1), amount: z.string().min(1), reason: z.string().trim().min(5).max(240) }).parse(Object.fromEntries(formData)); await updateManualJournal({ actor: { tenantId: active.id, userId: user.id, role: user.staffRole }, ...data }); revalidatePath("/"); revalidatePath("/journals"); revalidatePath("/reports"); redirect(`/journals/${data.journalId}`); }
