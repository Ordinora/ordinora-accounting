...
export default async function ChequePage({ params }: { params: Promise<{ kind: string; id: string }> }) {
  ...
  return <AppShell user={{ displayName: user.displayName, email: user.email, role: user.staffRole?.replaceAll("_", " ") ?? "STAFF", firmName: user.firm.name }} tenants={tenants} activeTenant={active} pageTitle={`Cheque ${payment.chequeNumber}`} pageDescription="Track cheque clearance or record a returned cheque">
    <main className="module-page"><header className="module-header"><div><p className="eyebrow">{payee.toUpperCase()}</p><h2>Bank cheque {payment.chequeNumber}</h2><p>The original payment remains visible for audit. A returned cheque creates the correcting journal automatically.</p></div><span className="status-pill">{payment.chequeStatus ?? "ISSUED"}</span></header>
      <section className="surface-card"><div className="report-summary-grid"><div><span>Reference</span><strong>{payment.reference}</strong></div><div><span>Cheque date</span><strong>{payment.chequeDate?.toLocaleDateString("en-BN")}</strong></div><div><span>Bank account</span><strong>{payment.bankAccount.code} — {payment.bankAccount.name}</strong></div><div><span>{"discountForeignAmount" in payment ? "Cheque amount" : "Amount"}</span><strong>{payment.currency} {Number(payment.foreignAmount).toFixed(2)}</strong></div>{"discountForeignAmount" in payment && payment.discountForeignAmount.gt(0) && <><div><span>Supplier discount</span><strong>{payment.currency} {Number(payment.discountForeignAmount).toFixed(2)}</strong></div><div><span>Total settled</span><strong>{payment.currency} {Number(payment.foreignAmount.add(payment.discountForeignAmount)).toFixed(2)}</strong></div></>}</div></section>
