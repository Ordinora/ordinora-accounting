...
export default async function NewPayment() {
  ...
  const [bankAccounts, postingAccounts, tenantCurrencies, inventoryItems, inventoryLocations, mappingAccounts, suppliers] = await Promise.all([
    ...
    db.supplier.findMany({ where: { tenantId: active.id, isActive: true }, orderBy: { name: "asc" }, select: { code: true, name: true } }),
  ]);
