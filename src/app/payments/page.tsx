...
export default async function PaymentsPage() {
  ...
  return <AppShell user={{ displayName: user.displayName, email: user.email, role: user.staffRole?.replaceAll("_", " ") ?? "STAFF", firmName: user.firm.name }} tenants={tenants} activeTenant={active} pageTitle="Payments" pageDescription="All money paid from cash and bank accounts">
    <main className="module-page">
      ...
      <section className="surface-card table-card"><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Reference</th><th>Payee</th><th>Date</th><th>Paid from</th><th>Type</th><th className="numeric">Amount</th><th aria-label="Options"></th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td><strong>{row.reference}</strong></td><td>{row.payee}</td><td>{row.date.toLocaleDateString("en-BN")}</td><td>{row.paidFrom}</td><td>{row.type}</td><td className="numeric">{row.currency} {Number(row.amount).toFixed(2)}</td><td><SourceRecordActions editHref={row.editHref} journalId={row.journalId} fixedAssetAction={row.fixedAssetAction}/></td></tr>)}{!rows.length && <tr><td colSpan={7} className="table-empty">No payments have been recorded.</td></tr>}</tbody></table></div></section>
    </main>
  </AppShell>;
}
