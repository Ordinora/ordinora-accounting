...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const document = await db.document.findFirst({ where: { id, tenantId: tenant.id, uploadedById: user.id } });
  if (!document) return new Response("Document not found", { status: 404 });
  if (blockedStatuses.includes(document.status as (typeof blockedStatuses)[number])) return new Response("This document is not available for download.", { status: 423 });
  try {
