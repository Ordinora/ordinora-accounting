...
export default async function ClientPortalPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string }> }) {
  ...
  return <main className="portal-page">
    ...
    <section className="portal-content">
      ...
      {liveAccess && <div className="kpi-grid">{tenant.enabledDashboardCards.map((key) => { const card = cards[key as keyof typeof cards]; if (!card) return null; const [label, value] = card; const periodCopy = key === "revenue" || key === "profit" ? `${range.fromInput} to ${range.toInput}` : `As of ${range.toInput}`; return <Link href={`/portal/live/${key}?from=${range.fromInput}&to=${range.toInput}`} className="kpi-card portal-kpi-link" key={key}><div className="kpi-label"><span>{label}</span></div><strong>{money(tenant.defaultCurrency, value)}</strong><p>{periodCopy} · Open drill-down</p></Link>; })}</div>}
