...
export async function GET(request: Request, { params }: { params: Promise<{ type: string }> }) {
  ...
  if (type === "trial-balance") {
    ...
    title = "Employee Summary"; subtitle = `For the period ${formatDate(from)} to ${formatDate(asOf)} | Posted and locked payroll`;
    ...
    sections = [{ title: "Payroll transactions", rows: [...entries.map(entry => ({ label: `${formatDate(entry.payrollRun.payDate)}  ${entry.payrollRun.reference}`, detail: `${entry.payrollRun.runType.replaceAll("_", " ")} | SPK ${amount(entry.employeeSpk)}`, amount: amount(entry.netPay) })), { label: "Gross pay", amount: amount(totals.gross), strong: true }, { label: "Employee SPK", amount: amount(totals.employeeSpk), strong: true }, { label: "Other deductions", amount: amount(totals.deductions), strong: true }, { label: "Net pay", amount: amount(totals.net), strong: true }] }];
  } else if (type === "payslip-totals") {
    const rows = payslipItemSummary(await payrollEntriesForPeriod(active.id, from, asOf)); title = "Payslip Totals by Item and Employee"; subtitle = `For the period ${formatDate(from)} to ${formatDate(asOf)} | Posted and locked payroll`;
