...
export default async function CurrencySettingsPage() {
  ...
  return <AppShell user={{ displayName:user.displayName,email:user.email,role:user.staffRole?.replaceAll("_"," ")??"STAFF",firmName:user.firm.name }} tenants={tenants} activeTenant={active} pageTitle="Currency Settings" pageDescription="Base currency and effective-dated exchange rates">
    <main className="module-page"><header className="module-header"><div><p className="eyebrow">{active.legalName.toUpperCase()}</p><h2>Multicurrency</h2><p>Foreign amounts are converted into {active.defaultCurrency} for the general ledger. Base-currency amounts display as {currencyDisplaySymbol(active.defaultCurrency)}.</p></div></header>
      {!active.multiCurrencyEnabled && <form action={enableMulticurrency} className="surface-card form-panel"><section className="form-section"><div className="section-heading"><h2>Enable multicurrency</h2><p>The ISO base-currency code becomes locked after posted transactions exist.</p></div><div className="form-grid"><label>Base currency code<input name="baseCurrency" defaultValue={active.defaultCurrency} minLength={3} maxLength={3} required /><small>{active.defaultCurrency} amounts display as {currencyDisplaySymbol(active.defaultCurrency)}.</small></label></div></section><div className="form-actions"><button className="button-primary">Enable multicurrency</button></div></form>}
