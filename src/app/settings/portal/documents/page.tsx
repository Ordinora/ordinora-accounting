...
import { updateDocumentWorkflow } from "./actions";

export const dynamic = "force-dynamic";

export default async function PortalDocumentInbox({ searchParams }: { searchParams: Promise<{ error?: string; updated?: string }> }) {
    ...
    <main className="module-page">
      ...
      <section className="surface-card table-card portal-document-inbox"><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Document</th><th>Uploaded by</th><th>Security</th><th>Processing</th><th>Received</th><th>Workflow update</th></tr></thead><tbody>{documents.map((document) => {
        ...
        return <tr key={document.id}>
          ...
          <td>{blocked || document.workflowStatus === "POSTED" ? <small>{blocked ? "Waiting for security clearance" : "Posted workflow completed"}</small> : <form action={updateDocumentWorkflow.bind(null, document.id)} className="document-workflow-form"><select name="workflowStatus" defaultValue="" required><option value="" disabled>Choose next status</option>{availableDocumentWorkflowStatuses(document.workflowStatus).map((status) => <option key={status} value={status}>{status.replaceAll("_", " ")}</option>)}</select><input name="workflowReference" maxLength={120} placeholder="Posting reference (required for Posted)"/><input name="workflowNote" maxLength={500} placeholder="Client-visible note / rejection reason"/><button className="button-secondary">Update</button></form>}</td>
        </tr>;
