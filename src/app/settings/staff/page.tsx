...
export default async function StaffSettingsPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  if (user.staffRole !== "SYSTEM_ADMIN") return <main className="login-page"><section className="login-card"><h1>System Administrator access required</h1><p>Only the System Administrator can create and manage accounting staff.</p></section></main>;
  const [{ tenants, active }, staff, query] = await Promise.all([
    getAuthorizedTenant(user),
    db.user.findMany({ where: { firmId: user.firmId, kind: "STAFF" }, include: { assignments: { include: { tenant: true } } }, orderBy: [{ staffRole: "asc" }, { displayName: "asc" }] }),
    searchParams,
  ]);

  const content = <main className="module-page form-page">
    <header className="module-header"><div><p className="eyebrow">SYSTEM ADMINISTRATION</p><h2>Accounting staff</h2><p>Create, update, deactivate, and reset credentials for operational users.</p></div></header>
    ...
    <section className="surface-card table-card"><div className="card-header"><div><h3>Staff register</h3><p>The System Administrator has all-company access; other staff see only assigned companies.</p></div></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Assigned companies</th><th>Status</th><th>Actions</th></tr></thead><tbody>{staff.map((member) => <tr key={member.id}><td><strong>{member.displayName}</strong>{member.id === user.id && <small className="table-subtext">Your account</small>}</td><td>{member.email}</td><td>{member.staffRole?.replaceAll("_", " ")}</td><td>{member.staffRole === "SYSTEM_ADMIN" ? "All companies" : member.assignments.map((assignment) => assignment.tenant.legalName).join(", ") || "None"}</td><td><span className={`status-badge ${member.isActive ? "active" : "inactive"}`}>{member.isActive ? "ACTIVE" : "INACTIVE"}</span></td><td><div className="workflow-actions"><Link className="record-link" href={`/settings/staff/${member.id}/edit`}>Edit</Link><Link className="record-link" href={`/settings/staff/${member.id}/edit#reset-password`}>Reset password</Link><Link className="record-link" href={`/settings/staff/${member.id}/edit#account-status`}>{member.isActive ? "Deactivate" : "Reactivate"}</Link></div></td></tr>)}</tbody></table></div></section>
  </main>;
