...
export function CompanyEditForm({ company }: { company: Company }) {
  ...
  return <form action={action} className="surface-card compact-form-body">
    ...
    <div className="form-grid">
      ...
      <label>Base currency code<input name="defaultCurrency" minLength={3} maxLength={3} required readOnly={company.journalCount > 0} defaultValue={company.defaultCurrency}/><small>{company.journalCount ? "Locked after posted accounting entries. The display symbol can differ from the ISO code." : "Use a three-letter ISO currency code. BND amounts display as B$."}</small></label>
