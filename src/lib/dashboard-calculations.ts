...
export type DashboardLedgerLine = { debit: Prisma.Decimal.Value; credit: Prisma.Decimal.Value; account: { code: string; type: string; reportingClassification: string } };
export function calculateDashboardBalances(lines: DashboardLedgerLine[]) {
  ...
  const receivables = lines.filter((line) => line.account.code === "1200").reduce((sum, line) => sum + signed(line), 0);
  const payables = Math.abs(lines.filter((line) => ["2000", "2100"].includes(line.account.code)).reduce((sum, line) => sum + signed(line), 0));
