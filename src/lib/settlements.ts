...
type SettlementInput={kind:"RECEIPT"|"PAYMENT";actor:Actor;partyId:string;bankAccountId:string;reference:string;settlementDate:Date;allocations:AllocationInput[]} & Partial<ChequeDetails>;
...
export async function postSettlement(input:SettlementInput){
  ...
  return runSerializableTransaction(async tx=>{
    ...
    const cheque=input.kind==="PAYMENT"?validateChequeDetails({paymentMethod:input.paymentMethod??"BANK_TRANSFER",chequeNumber:input.chequeNumber,chequeDate:input.chequeDate}):null;
